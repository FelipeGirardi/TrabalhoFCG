...
float save_cameraTheta_free = 0.0f;
float save_cameraPhi_free = 0.0f;
float save_cameraDistance_free = 10.0f;
...
int main(int argc, char* argv[])
{
    ...
    ObjModel romanmodel("../../data/romanEmperor.obj");
    ComputeNormals(&romanmodel);
    BuildTrianglesAndAddToVirtualScene(&romanmodel);

    ObjModel palmmodel("../../data/MY_PALM.obj");
...
//    glm::mat4 the_view;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //
        //           R     G     B     A
        glClearColor(161.0f/255.0f, 121.0f/255.0f, 176.0f/255.0f, 1.0f);
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mod)
{
    ...
    if (key == GLFW_KEY_C && action == GLFW_PRESS)
    {
        ...
        if(g_UseLookAtCamera) {
            ...
            save_cameraPhi_lookAt = g_CameraPhi;
            save_cameraDistance_lookAt = g_CameraDistance;
            g_CameraTheta = save_cameraTheta_free;
            g_CameraPhi = save_cameraPhi_free;
            g_CameraDistance = save_cameraDistance_free;
            camera_position_c = save_camera_position_free;
        }
        else {
            g_UseLookAtCamera = true;
            save_cameraTheta_free = g_CameraTheta;
            save_cameraPhi_free = g_CameraPhi;
            save_cameraDistance_free = g_CameraDistance;
            save_camera_position_free = camera_position_c;
            g_CameraTheta = save_cameraTheta_lookAt;
            g_CameraPhi = save_cameraPhi_lookAt;
            g_CameraDistance = save_cameraDistance_lookAt;
        }
    }
